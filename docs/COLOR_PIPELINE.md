...
Lab eyedropper analysis follows the same criterion. ProbRAW can convert image
RGB to Lab when an ICC is assigned, but the reading is a rigorous colorimetric
measurement only when that ICC is a ProbRAW-generated session profile for the
same camera, recipe and illuminant. With generic ICC profiles, Lab and DeltaE are
informative diagnostics of the assigned space.
...
12. The 3D/Lab 2D gamut diagnostic is a visual profile comparison; it does not
    modify recipes, pixels, active profiles or manifests.
...
14. Lab eyedropper and sample-set comparisons must use real pixels from the
    full-resolution loaded image; colorimetric measurements are not accepted from
    thumbnails or reduced previews.
15. Samples are saved per image in the RAW backpack and compared by sets. There
    is no primary reference sample inside a set; the comparative reference is
...
6. Pixel Lab sampling uses the same separation: image RGB with input ICC for
   analysis, and monitor conversion only for display.
...
`PREVIEW_AUTO_BASE_MAX_SIDE`; 1:1 precision, compare, chart marking, pixel Lab
sampling and MTF measurements force full resolution when real coordinates are
needed. During 100% work, interactions apply adjustments to the visible crop,
update viewer regions and reuse dense ICC LUT caches generated by LittleCMS so
performance does not sacrifice colorimetric precision. Tone curves reuse tonal
LUTs and share RGB quantization before the `source ICC -> monitor ICC` and
instrument conversions.

## Lab Sample Diagnostics

The `Diagnóstico > Muestras` tab is diagnostic and does not alter the development
pipeline. Each sample records real coordinate, sampling matrix, normalized RGB,
Lab, C*, gamut membership and the ICC used to interpret RGB. Samples are grouped
into sets to represent internal variation of an ink, paper, area or material.

Set comparisons use:

- mean RGB and Lab per set;
- internal `DE00` dispersion against the set center;
- DeltaE and C* difference against the active reference set;
- similarity percentage based on nearest neighbours at `DE00 <= 3`;
- visual sample gamut in Lab a*b*.

These data are valid for comparative audit when they come from an image with a
ProbRAW-generated session ICC. With generic profiles, they must be labelled as
