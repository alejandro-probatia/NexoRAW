...
ProbRAW 0.4.4 es adecuado para pruebas controladas, revisión metodológica y
validación de candidata a release. Todavía no es un sistema certificado para
producción científica o forense.

La version actual refuerza el flujo ICC y las lecturas colorimetricas sin perder
respuesta en previews interactivas: mantiene la generacion de perfiles sobre
RAW/DNG originales, permite revisar zonas centrales de lectura, exige pixel real
para muestras Lab y evita renders completos innecesarios durante ajustes de
viewport.

La última validación de empaquetado pasó con:

```text
La validacion de release esta documentada en docs/releases/0.4.4.es.md.
